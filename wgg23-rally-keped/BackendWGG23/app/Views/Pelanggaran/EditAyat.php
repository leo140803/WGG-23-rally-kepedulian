...
<nav class="navbar navbar-expand-lg bg-body-tertiary">
  ...
  <div class="inputBox d-flex justify-content-center align-items-center mt-5 pt-5">
    <?= form_open(site_url("/panitia/pelanggaran/Edit")); ?>

    <div class="input">
      <label> Masukkan Keterangan Ayat: </label>
      <input type="text" name="keteranganAyat" value="<?= $fetch_data->Keterangan ?>"  class="form-control mb-3" required>
      
      <label> Pilih Pasal: </label>
      <select name="pasalAyat" class="form-select mb-3" required>
        <option value="<?= $fetch_data->Pasal ?>"> <?= $fetch_data->Pasal, " (Unedited)" ?> </option>
        ...
        <select name="sistemTegur" class="form-select mb-3" required>
          <option value="<?= $fetch_data->sistemTegur ?>">
          <?php if( $fetch_data->sistemTegur == 0): ?>
            <?= "Tidak (Unedited)" ?>
          <?php else: ?>
            <?= "Ya (Unedited)" ?>
          <?php endif ?>
        ...
        <!-- <input type="text" name="nomorAyat" value="<//?= $fetch_data->No ?>" required> -->
    </div>
