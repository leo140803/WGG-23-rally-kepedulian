    ...
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav">
          ...
          <li class="nav-item">
            <a class="nav-link active" href="<?= site_url("/panitia/pelanggaran/pasalList") ?>">Data Pasal</a>
          </li>
          <li class="nav-item">
      ...
      <table class="display" id="tbl">
        ...
        <tbody>
          <?php foreach ($dataPasal as $pasal): ?>
            <tr>
              <td>
                <?= htmlspecialchars($pasal->Bab)?>
              </td>
              <td>
                  <?= htmlspecialchars($pasal->Keterangan) ?>
              </td>
              <td>
                <?= htmlspecialchars($pasal->JumlahPoin) ?>
              </td>
              <td>
                <a href="<?= site_url('/panitia/pelanggaran/pasalEdit' . $pasal->Bab) ?>" class="btn btn-secondary btn-sm">Edit</a>
                <button name="hapusData" class="hapus_data btn btn-danger btn-sm">Hapus</button>
                <?= form_open(site_url('/panitia/pelanggaran/pasalHapus/' . $pasal->id),
                              ['class' => 'confirmdelete', 'method' => 'post']
                              ) ?>
                              <input type="hidden" name="_method" value="delete">
                <?= form_close() ?>
                <!-- <a href="<//?= site_url('/panitia/pelanggaran/pasalHapus' . $pasal->Bab) ?>" class="btn btn-danger btn-sm">Hapus</a> -->
              </td>
            </tr>
          <?php endforeach ?>
        </tbody>
